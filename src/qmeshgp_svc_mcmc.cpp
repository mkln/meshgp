...
Rcpp::List qmeshgp_svc_mcmc(
  ...
  arma::mat b_mcmc = arma::zeros(X.n_cols, mcmc_keep * mcmc_thin);
  arma::mat tausq_mcmc = arma::zeros(1, mcmc_keep * mcmc_thin);
  arma::vec sigmasq_mcmc = arma::zeros(mcmc_keep * mcmc_thin);
  arma::mat theta_mcmc = arma::zeros(npars, mcmc_keep * mcmc_thin);
  arma::vec llsave = arma::zeros(mcmc_keep * mcmc_thin);
  ...
  try { 
    for(m=0; m<mcmc; m++){
      ...
      //save
      if(mx >= 0){
        tausq_mcmc.col(mx) = 1.0 / mesh.tausq_inv;
        b_mcmc.col(mx) = mesh.Bcoeff;
        sigmasq_mcmc(mx) = mesh.sigmasq;
        theta_mcmc.col(mx) = mesh.param_data.theta;
        llsave(mx) = current_loglik;
        if(mx % mcmc_thin == 0){
...
//[[Rcpp::export]]
Rcpp::List qmeshgp_collapsed_mcmc(
    const arma::mat& y, 
    const arma::mat& X, 
    const arma::mat& Z,
  ...
  arma::vec theta_for_collapsed = arma::join_vert(arma::join_vert(
    arma::ones(1) / tausq, arma::ones(1) * sigmasq
  ), theta);
  
  npars = theta_for_collapsed.n_elem;
  arma::mat set_unif_bounds = arma::zeros(npars, 2);
  for(int i=0; i<npars; i++){
    set_unif_bounds.row(i) = set_unif_bounds_in.row(0);
  }
  ...
  if(recover_mesh){
    ...
    mesh = MeshGPsvc(y, X, Z, coords, blocking,
                     ...
                     start_w, beta, theta_for_collapsed, 1.0/tausq, 
                     sigmasq, matern_twonu,
                     beta_Vi, sigmasq_ab, tausq_ab,
  ...
  mesh.get_loglik_comps_y_collapsed( mesh.param_data );
  mesh.get_loglik_comps_y_collapsed( mesh.alter_data );
  ...
  MHAdapter adaptivemc(param.n_elem, mcmc, arma::eye(npars, npars) * mcmcsd(0,0));
  ...
  try { 
    ...
    return Rcpp::List::create(
      ...
      Rcpp::Named("mcmc_time") = mcmc_time/1000.0,
      Rcpp::Named("recover") = recovered,
      Rcpp::Named("debug") = mesh.debug_stuff
    );
